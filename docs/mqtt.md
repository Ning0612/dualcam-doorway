...
> **注意**：Broker 連線成功 ≠ Agent 2 在線。Agent 2 在線狀態由 presence heartbeat 的到達時間決定，而非 MQTT broker 連線狀態。
...
| `home/home_state/alarm_decision` | Agent 2 | 警報決定回應 |
...
> **已知風險 2**：`CANCEL_ALARM` 到達時若 Yellow alert 尚在等待中（`_waitingForDecision=true` 但 alarm 尚未啟動），會清除等待狀態，使 30 秒逾時升級機制失效。
...
Agent 2 傳來的 presence / alarm_decision 訊息格式由 Agent 2 自行決定；FaceGuard 只解析 `presence_state`、`presence_score`、`alarm_decision` 欄位，其餘欄位不影響解析。
  ...
  if (!_client.connected()):
      if (now - _lastReconnectMs >= MQTT_RECONNECT_MS):
          _reconnect()
              ...
              → 成功: subscribe presence, alarm_decision, display/status topics
              → 失敗: 5s 後重試
```
